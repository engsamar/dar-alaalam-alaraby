            ...
            <div class="card">
                <div class="card-body">
                    ...
                    @if ($item->id)
                        ...
                        @else
                            <form class="needs-validation" action="{{ route('admin.colors.store') }}" method="POST"
                    ...
                    <div class="row mb-3">
                        <label class="col-form-label col-lg-2" for="color_code-field">@lang('attributes.color_code')</label>
                        <div class="col-lg-10">
                            <div class="form-group">
                                <input type="color" required
                                    class="form-control @if ($errors->has('color_code')) is-invalid @endif" name="color_code"
                                    value="{{ old('color_code', $item->color_code) ?? '0' }}" />
                                @if ($errors->has('color_code'))
                                    <span class="invalid-feedback">{{ $errors->first('color_code') }}</span>
                                @endif
                            </div>
                        </div>
                    </div>
