...
@section('tab_name', trans('titles.tags'))
@section('css')
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <hr>
                    @if ($item->id)
                        <form class="needs-validation" action="{{ route('admin.tags.update', $item->id) }}"
                            method="POST" enctype='multipart/form-data' novalidate>
                            <input type="hidden" name="_method" value="PUT">
                        @else
                            <form class="needs-validation" action="{{ route('admin.tags.store') }}" method="POST"
                        ...
                        <div class="col-lg-10">
                            <div class="form-group">
                                <select class="form-control select2" name="type">
                                    <option value="{{ \App\Helpers\Constants::ARTICLE_TAG_TYPE }}" {{ old('type', $item->type) == \App\Helpers\Constants::ARTICLE_TAG_TYPE ? 'selected' : '' }}>
                                        @lang('attributes.article')</option>
                                    <option value="{{ \App\Helpers\Constants::PRODUCT_TAG_TYPE }}" {{ old('type', $item->type) == \App\Helpers\Constants::PRODUCT_TAG_TYPE ? 'selected' : '' }}>
                                        @lang('attributes.product')</option>
                    ...
                    <div class="col-md-12">
                        ...
                        <a class="btn btn-danger pull-right text-white" style="float: right;"
                            href="{{ route('admin.tags.index') }}">@lang('common.cancel')
