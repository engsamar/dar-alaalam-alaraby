...
@section('tab_name', trans('titles.catalogues'))

@section('content')
    @include('admin.layouts.title', [
        'subTitle' => $item->id ? trans('common.edit') . '#' . $item->id : trans('common.add'),
        'title' => trans('titles.catalogues'),
    ])
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    ...
                    @if ($item->id)
                        <form class="needs-validation" action="{{ route('admin.catalogues.update', $item->id) }}"
                            method="POST" enctype='multipart/form-data' novalidate>
                            <input type="hidden" name="_method" value="PUT">
                        @else
                            <form class="needs-validation" action="{{ route('admin.catalogues.store') }}" method="POST"
                            ...
                            <div class="form-group">
                                <select class="form-control select2" name="active">
                                    <option value="1" {{ old('active', $item->active) == 1 ? '' : 'selected' }}>
                                        @lang('attributes.publish')</option>

                                    <option value="0" {{ old('active', $item->active) != 1 ? '' : 'selected' }}>
                                        @lang('attributes.draft')</option>
                    ...
                    <div class="col-md-12">
                        ...
                        <a class="btn btn-danger  {{ $locale == 'en' ? 'pull-right' : 'pull-left' }} text-white" style="float:{{ $locale == 'en' ? 'right' : 'left' }} "
                            href="{{ route('admin.catalogues.index') }}">@lang('common.cancel')
