...
@section('tab_name', trans('titles.features'))

@section('content')
    @include('admin.layouts.title', [
        'subTitle' => $item->id ? trans('common.edit') . '#' . $item->id : trans('common.add'),
        'title' => trans('titles.features'),
    ])
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    ...
                    @if ($item->id)
                        <form class="needs-validation" action="{{ route('admin.features.update', $item->id) }}" method="POST"
                            enctype='multipart/form-data' novalidate>
                            <input type="hidden" name="_method" value="PUT">
                        @else
                            <form class="needs-validation" action="{{ route('admin.features.store') }}" method="POST"
                                enctype='multipart/form-data' novalidate>
                    @endif
                    @csrf
                    ...
                    <div class="col-md-12">
                        ...
                        <a class="btn btn-danger  {{ $locale == 'en' ? 'float-end' : 'pull-left' }} text-white" style="float:{{ $locale == 'en' ? 'right' : 'left' }} "
                            href="{{ route('admin.features.index') }}">@lang('common.cancel')
