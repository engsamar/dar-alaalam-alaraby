        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    ...
                    @if ($item->id)
                        <form class="needs-validation" action="{{ route('admin.cities.update', $item->id) }}" method="POST"
                            enctype='multipart/form-data' novalidate>
                            <input type="hidden" name="_method" value="PUT">
                        @else
                            <form class="needs-validation" action="{{ route('admin.cities.store') }}" method="POST"
                                enctype='multipart/form-data' novalidate>
                    @endif
                    @csrf
                    ...
                    <div class="col-md-12">
                        ...
                        <a class="btn btn-danger  {{ $locale == 'en' ? 'float-end' : 'pull-left' }} text-white" style="float:{{ $locale == 'en' ? 'right' : 'left' }} "
                            href="{{ route('admin.cities.index') }}">@lang('common.cancel')
